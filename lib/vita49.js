...
class VitaReceiver extends EventEmitter {
  constructor(port) {
    ...
    this._byteOrderBE = false; // true if payload is big-endian (auto-detected from first packet)
  }
  ...
  /**
   * Auto-detect payload byte order by testing first samples as LE and BE.
   * Valid IQ float32 samples are finite with magnitude in [1e-6, 1e8].
   * Wrong byte order produces denormalized, huge, or NaN values.
   */
  _detectByteOrder(buf, payloadStart, numFloats) {
    const testCount = Math.min(20, numFloats);
    let leScore = 0, beScore = 0;

    for (let i = 0; i < testCount; i++) {
      const offset = payloadStart + i * 4;
      const leVal = buf.readFloatLE(offset);
      const beVal = buf.readFloatBE(offset);

      const leAbs = Math.abs(leVal);
      if (isFinite(leVal) && leAbs > 1e-6 && leAbs < 1e8) leScore++;

      const beAbs = Math.abs(beVal);
      if (isFinite(beVal) && beAbs > 1e-6 && beAbs < 1e8) beScore++;
    }

    // Require clear winner (margin of 3+), default to LE if ambiguous
    const useBE = beScore > leScore + 3;
    ...
    const rawHex = buf.slice(payloadStart, payloadStart + 8).toString('hex');

    this.emit('byte-order', {
      order: useBE ? 'BE' : 'LE',
      leScore, beScore, testCount,
      leI: leVal0, leQ: leVal1,
      beI: beVal0, beQ: beVal1,
      rawHex,
    });
  ...
  _parsePacket(buf) {
    ...
    // Auto-detect byte order from first packet
    if (this._packetsReceived <= 1) {
      this._byteOrderBE = this._detectByteOrder(buf, payloadStart, numFloats);
      console.log(`[VITA-49] Using ${this._byteOrderBE ? 'big' : 'little'}-endian payload`);
      console.log(`[VITA-49] Payload: ${numFloats} floats (${numFloats / 2} IQ pairs), ${payloadBytes} bytes`);
    }

    // Read float32 samples in detected byte order
    if (this._byteOrderBE) {
      for (let i = 0; i < numFloats; i++) {
        samples[i] = buf.readFloatBE(payloadStart + i * 4);
      }
    } else {
      for (let i = 0; i < numFloats; i++) {
