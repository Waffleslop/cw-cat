...
class CwEnvelopeDetector {
  ...
  constructor(rate) {
    ...
    this._sampleCount = 0;
    this._warmup = true;
    this._warmupSamples = Math.max(4, Math.ceil(rate * 0.025)); // 25ms warmup (was 50ms)
    ...
    for (let i = 0; i < magnitudes.length; i++) {
      ...
      if (this._minWindowFilled || this._minWindowPos >= 8) {
        ...
        if (targetNoise < this._noiseMag) {
          this._noiseMag = targetNoise; // Fast drop — immediately track lower noise
        } else if (this._smoothMag < this._threshold || this._threshold === 0) {
          // Use faster alpha during early samples for quicker convergence
          const alpha = this._sampleCount < this._rate ? this._noiseAlpha * 4 : this._noiseAlpha;
          this._noiseMag = this._noiseMag * (1 - alpha) + targetNoise * alpha;
        }
      } else {
