...
class CwEnvelopeDetector {
  ...
  constructor(rate) {
    ...
    //   smooth: 15ms (fast edge detection)
    //   avg: 100ms (diagnostic)
    //   peak/noise: 1s (slow tracking)
    this._smoothAlpha = 1 - Math.exp(-1 / (rate * 0.008)); // τ=8ms → α≈0.74@94Hz, α≈0.15@800Hz
    this._alpha = 1 - Math.exp(-1 / (rate * 0.1));          // τ=100ms
    this._peakAlpha = 1 - Math.exp(-1 / (rate * 3.0));      // τ=3s — slow decay preserves DR through word gaps
    this._noiseAlpha = 1 - Math.exp(-1 / (rate * 3.0));     // τ=3s
    this._state = false;     // Current key state: true = key down
    this._threshold = 0;     // Adaptive threshold
    this._sampleCount = 0;
    this._warmup = true;
    this._warmupSamples = Math.max(10, Math.ceil(rate * 0.1));  // 100ms warmup
    this._debounceCount = 0;
    this._debounceLimit = Math.max(1, Math.ceil(rate * 0.005)); // ~5ms debounce
  ...
  process(magnitudes, sampleRate) {
    ...
    for (let i = 0; i < magnitudes.length; i++) {
      ...
      // Need sufficient dynamic range to detect keying (at least 1.5:1)
      const dynamicRange = this._peakMag / (this._noiseMag + 1e-10);
      if (dynamicRange < 1.5) continue; // Not enough contrast to detect CW

      // Hysteresis — use absolute thresholds based on noise/peak levels
      // Wider hysteresis (0.65/0.35) compensates for faster smoothing being noisier
      const upperThresh = this._noiseMag + (this._peakMag - this._noiseMag) * 0.65;
      const lowerThresh = this._noiseMag + (this._peakMag - this._noiseMag) * 0.35;
