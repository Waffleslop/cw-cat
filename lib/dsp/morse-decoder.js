...
class MorseDecoder {
  ...
  _processOnDuration(duration) {
    ...
    // At 60 WPM, a dit = 20ms. Reject anything under 10ms.
    if (duration < 0.010) return;
    ...
    const minPlausible = this._speedLocked ? this._ditDuration * 0.6 : this._ditDuration * 0.2;
    const maxPlausible = this._speedLocked ? this._ditDuration * 4.0 : this._ditDuration * 8.0;
  ...
  _processOffDuration(duration) {
    ...
    if (duration > this._ditDuration * 15) {
      // Buffer the raw off-duration before speed lock for later re-classification
      if (!this._speedLocked && this._preBuffer.length < this._maxPreBuffer) {
        this._preBuffer.push({ type: 'off', duration });
      }
    ...
    // Buffer ALL off-durations before speed lock — they get re-classified during replay
    // using the correct dit duration (critical: initial dit estimate may be wrong,
    // causing char gaps to be misclassified as intra-char, merging characters)
    if (!this._speedLocked && this._preBuffer.length < this._maxPreBuffer) {
      this._preBuffer.push({ type: 'off', duration });
    }
  ...
  _updateSpeedEstimate() {
    ...
    if (!this._speedLocked && this._onDurations.length >= 5) {
      ...
      if (consistent) {
        ...
        if (fastWpm >= this._minWpm && fastWpm <= this._maxWpm) {
          // Check if there's at least one element >1.5x the median (likely a dah)
          // Lowered from 1.8x to 1.5x — helps lock faster on stations with tight dah/dit ratio
          const hasLonger = sorted5.some(d => d > median * 1.5);
          if (hasLonger) {
    ...
    // Both clusters must have sufficient elements for initial lock
    // Dits need ≥3 for reliable median; dahs need ≥1 to confirm bimodal distribution
    if (!this._speedLocked && (ditClusterSize < 3 || dahClusterSize < 1)) return;
   ...
   * Re-classifies ALL raw on/off durations using correct thresholds.
   * This is critical because the initial dit estimate (80ms) may differ wildly
   * from the actual speed, causing char gaps to be missed during initial collection.
   */
  _replayPreBuffer() {
    if (this._preBuffer.length === 0) return;

    const ditThreshold = this._ditDuration * Math.sqrt(this._dahDitRatio);
    // Compute gap thresholds using now-known dit duration
    const charGap = this._ditDuration * 2.5;
    const wordGap = this._ditDuration * 5.5;
    let elements = [];

    for (const entry of this._preBuffer) {
      if (entry.type === 'on') {
        // Re-classify using now-known dit duration
        if (entry.duration < ditThreshold) {
          elements.push('.');
        } else {
          elements.push('-');
        }
      } else if (entry.type === 'off') {
        // Re-classify gap using now-known dit duration
        if (entry.duration >= wordGap || entry.duration > this._ditDuration * 15) {
          // Word gap — finalize character, add space
          ...
          this._decodedText += ' ';
          if (this._onWord) this._onWord(' ');
        } else if (entry.duration >= charGap) {
          // Character gap — finalize character
          if (elements.length > 0) {
            const morse = elements.join('');
            const char = MORSE_TABLE[morse] || '\u00BF';
        ...
        // Else: intra-character gap — elements keep accumulating
      }
    }
