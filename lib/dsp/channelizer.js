...
//   Stage 2: FIR lowpass at 8kHz with 127 taps → 15.9ms filter span → effective narrowband
...
class CwChannel {
  ...
  constructor(freqOffset, inputRate, outputRate, bandwidth) {
    ...
    // 255 taps at 8kHz = 31.9ms span, group delay = 15.9ms
    // Blackman window: transition BW ≈ 376Hz, stopband rejection ≈ 58dB
    // At 200Hz offset: ~20dB rejection. At 400Hz: ~45dB. At 500Hz+: >58dB.
    // Previous 63 taps had transition BW of 1524Hz — useless for CW isolation
    this._filterLen = 255;
  ...
  _designLpf(cutoff, sampleRate, numTaps) {
    ...
    for (let i = 0; i <= M; i++) {
      ...
      // Blackman window (better stopband than Hamming: ~58dB vs ~43dB)
