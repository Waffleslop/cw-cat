...
class SignalDetector {
  ...
  /**
   * Reset noise floor and tracking state (e.g., after a band change).
   * Forces re-warmup to build a fresh noise floor estimate.
   */
  reset() {
    this._noiseFloorInitialized = false;
    this._frameCount = 0;
    this._persistence.fill(0);
    this._activeSignals.clear();
    this._trackedSignals.clear();
  }
