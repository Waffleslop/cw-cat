...
class CallsignExtractor {
  ...
  extract(text) {
    ...
    for (const pattern of CQ_PATTERNS) {
      ...
      if (m && m[1]) {
        ...
        if (!seen.has(call) && this._isValidCallsign(call)) {
          ...
          // If portable callsign, also check that base call appears
          // This prevents garbled suffixes like W1AJT/N from random chars after /
          if (call.includes('/')) {
            const base = call.split('/')[0];
            const baseCount = this._countCallsignOccurrences(upper, base);
            if (baseCount < 2) continue; // Base call must also be frequent
          }
    ...
    // Fallback: if no context-based matches, look for callsigns repeated 3+ times
    // High repetition count compensates for missing CQ/DE context words
    // Require callsign to appear as a standalone word (preceded by space/start)
    // to avoid false matches from concatenated CW words like "TUN6NT" (TU + N6NT)
    if (results.length === 0) {
      let match;
      // Require space or start-of-string before callsign (not concatenated with CW words)
      const standaloneRe = /(?:^|[\s])([A-Z0-9]{1,3}[0-9][A-Z]{1,4}(?:\/[A-Z0-9]{1,4})?)\b/g;
      while ((match = standaloneRe.exec(upper)) !== null) {
        const call = match[1];
        if (!seen.has(call) && this._isValidCallsign(call)) {
          // Count standalone occurrences (with space before them)
