...
// CQ calling patterns — each has a 'concat' flag for concatenated (no-space) variants
// Concatenated patterns require standalone validation to prevent false positives
const CQ_PATTERNS = [
  // "CQ CQ CQ DE W1AW W1AW K" or "CQ CQ DE K2SX/ME"
  { re: new RegExp('CQ\\s+(?:CQ\\s+)*(?:DX\\s+)?DE\\s+' + CALL_PAT), concat: false },
  // "CQ CQ W1AW W1AW"
  { re: new RegExp('CQ\\s+(?:CQ\\s+)*(?:DX\\s+)?' + CALL_PAT), concat: false },
  // "DE W1AW" or "DE W1AW/4"
  { re: new RegExp('DE\\s+' + CALL_PAT), concat: false },
  // "TEST W1AW" or "TEST DE W1AW"
  { re: new RegExp('TEST\\s+(?:DE\\s+)?' + CALL_PAT), concat: false },
  // Concatenated CQ (common decode artifact): "CQDEWB8ZUR" or "CQCQDEW1AW"
  { re: new RegExp('CQ(?:CQ)*DE' + CALL_PAT), concat: true },
  // Concatenated DE (no space): "DEKE9BHN" or "DEW1AW"
  { re: new RegExp('DE' + CALL_PAT), concat: true },
  // Garbled TEST: "NST" "EST" "IEST" "TIST" followed by callsign (common T→N/I misdecodings)
  { re: new RegExp('[NEIT]ST\\s+' + CALL_PAT), concat: false },
  // Contest CQ: "CQ NCQP W8FN" or "CQ POTA KE9BHN" — CQ + contest/activity name + callsign
  { re: new RegExp('CQ\\s+[A-Z]{2,6}\\s+' + CALL_PAT), concat: false },
  // Concatenated contest CQ: "CQNCQPNC4KW" — CQ + junk + callsign (no spaces)
  { re: new RegExp('CQ[A-Z]{2,6}' + CALL_PAT), concat: true },
];
...
class CallsignExtractor {
  ...
  extract(text, snr) {
    ...
    for (const { re: pattern, concat } of CQ_PATTERNS) {
      ...
      if (m && m[1]) {
        ...
        if (!seen.has(call) && this._isValidCallsign(call) && !this._isFalsePositive(call)) {
          ...
          // For concatenated patterns (no spaces), require standalone appearance
          // to reject artifacts from repeated callsigns like "K8ACK8ACK8AC"
          // Spaced patterns already have word boundaries so don't need this check
          if (concat) {
    ...
    if (results.length === 0) {
      ...
      while ((match = standaloneRe.exec(upper)) !== null) {
        ...
        if (!seen.has(call) && this._isValidCallsign(call) && !this._isFalsePositive(call)) {
          ...
          // Strong signals (SNR>=20): 2x standalone is enough (cty.dat validates)
          // Dominant callsign: 3x (clearly repeated)
          // Weak signals: 4x (higher false positive risk)
          let standaloneMin = 4;
          if (snr !== undefined && snr >= 20) standaloneMin = 2;
          else if (dominatesText) standaloneMin = 3;
  ...
  _countStandaloneOccurrences(text, callsign) {
    ...
    let pos = 0;
    while ((pos = text.indexOf(searchStr, pos)) !== -1) {
      // Must be preceded by space, start of string, ¿, or > (prosign like <BT>)
      const prev = text[pos - 1];
      if (pos === 0 || prev === ' ' || prev === '\u00BF' || prev === '>') {
        count++;
      }
      pos += searchStr.length;
    }
