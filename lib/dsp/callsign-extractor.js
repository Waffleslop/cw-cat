...
class CallsignExtractor {
  ...
  extract(text, snr) {
    ...
    // strong signals with context can be trusted with fewer
    const minOccurrences = (snr !== undefined && snr < 15) ? 3 :
                           (snr !== undefined && snr >= 25) ? 1 : 2;
        ...
        for (const pattern of qsoPatterns) {
          ...
          if (m) {
            ...
            for (let gi = 1; gi <= 2 && gi < m.length; gi++) {
              const call = m[gi];
              if (call && !seen.has(call) && this._isValidCallsign(call)) {
                const callCount = this._countCallsignOccurrences(upper, call);
    ...
    // Fallback: callsigns repeated 3+ times standalone (no CQ/DE context)
    if (results.length === 0) {
      let match;
      ...
      while ((match = standaloneRe.exec(upper)) !== null) {
        ...
        if (!seen.has(call) && this._isValidCallsign(call)) {
          ...
          // Require 4x for fallback to reduce false positives (was 3)
          if (callCount >= 4) {
