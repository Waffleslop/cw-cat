// RBN telnet client — adapted for CW CAT as a spot PRODUCER (skimmer node)
// Connects to RBN aggregator and submits decoded CW spots
const net = require('net');
const { EventEmitter } = require('events');

const DEFAULT_HOST = 'arcluster.reversebeacon.net';
...
class RbnClient extends EventEmitter {
  ...
  /**
   * Connect to RBN aggregator as a skimmer node.
   * @param {object} opts
   * @param {string} opts.host - RBN aggregator host
   * @param {number} opts.port - RBN aggregator port
   * @param {string} opts.callsign - Operator callsign (will append -# for skimmer ID)
   */
  ...
  _processLine(line) {
    ...
    // Log any server messages for diagnostics
    if (line.trim()) {
  ...
  _handleLogin(line) {
    ...
    if (lower.includes('login:') || lower.includes('call:') || lower.includes('callsign:') ||
      ...
      if (this._target.callsign && !this._loggedIn) {
        // Login with callsign-# suffix (skimmer convention)
        const loginCall = this._target.callsign + '-#';
        this._write(loginCall + '\r\n');
        this._loggedIn = true;
        this._buf = '';
        this._startKeepalive();
        this.emit('logged-in', loginCall);
      }
    }
  }

  /**
   * Submit a DX spot to the RBN aggregator.
   * Format: DX de <MY_CALL>-#: <freq_kHz> <DX_CALL> <comment> <HHMM>Z
   */
