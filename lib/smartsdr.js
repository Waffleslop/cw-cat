...
class SmartSdrClient extends EventEmitter {
  constructor() {
    ...
    this._cwBreakIn = true; // Default true — allow TX until radio explicitly says otherwise
  }

  connect(host, port) {
    this.disconnect();
    this._host = host || '127.0.0.1';
    this._port = port || 4992;
    this._doConnect();
  }
  ...
  _parseStatusMessage(line) {
    ...
    if (panMatch) {
      ...
      // Log first pan update to confirm key names
      const existing = this._panadapters.get(panId);
      if (!existing && params.center) {
        console.log(`[SmartSDR] Pan ${panId} first center: ${params.center} MHz`);
      }
      const merged = { ...(existing || {}), ...params, id: panId };
      this._panadapters.set(panId, merged);
      this.emit('panadapter', merged);
      return;
    }

    // Parse interlock status: S<handle>|interlock <key=value ...>
    const interlockMatch = line.match(/\|interlock\s+(.*)/);
    if (interlockMatch) {
      const params = this._parseKeyValue(interlockMatch[1]);
      if (params.cw_break_in !== undefined) {
        this._cwBreakIn = params.cw_break_in === '1';
      }
      return;
    }
    ...
    const tryNext = (idx) => {
      ...
      this._sendWithCallback(cmd, (status, message) => {
        if (status === 0) {
          ...
          // Try multiple rate-setting commands — API varies by firmware version
          // Some accept kHz shorthand, others require Hz, others use channel-based commands
          this._setDaxIqRate(channel, rate);
  ...
  /**
   * Try multiple commands to set the DAX IQ sample rate.
   * FlexRadio API varies by firmware — some accept kHz, some Hz, some use channel commands.
   */
  _setDaxIqRate(channel, rate) {
    const rateKhz = Math.round(rate / 1000);
    const rateAttempts = [
      { cmd: `stream set ${this._streamId} daxiq_rate=${rateKhz}`, desc: `daxiq_rate=${rateKhz} (kHz)` },
      { cmd: `stream set ${this._streamId} daxiq_rate=${rate}`, desc: `daxiq_rate=${rate} (Hz)` },
      { cmd: `dax iq set ${channel} rate=${rate}`, desc: `dax iq set rate=${rate}` },
    ];

    const tryRate = (idx) => {
      if (idx >= rateAttempts.length) {
        console.log(`[SmartSDR] WARNING: All rate-setting commands failed — radio may use default rate`);
        return;
      }
      const { cmd, desc } = rateAttempts[idx];
      console.log(`[SmartSDR] Setting rate: ${cmd}`);
      this._sendWithCallback(cmd, (status) => {
        if (status === 0) {
          console.log(`[SmartSDR] DAX IQ rate set OK: ${desc}`);
        } else {
          console.log(`[SmartSDR] Rate command failed: ${desc} (0x${status.toString(16)}), trying next...`);
          tryRate(idx + 1);
        }
      });
    };
  ...
  /**
   * Get the center frequency of the DAX IQ panadapter in MHz.
   * This is the actual center of the DAX IQ data stream.
   * Prefers the panadapter with daxiq_channel assigned; falls back to first with a center.
   */
  getPanCenter() {
    // First try: find the pan with DAX IQ channel assigned
    for (const [, pan] of this._panadapters) {
      if (pan.daxiq_channel && parseInt(pan.daxiq_channel) > 0) {
        const center = parseFloat(pan.center || 0);
        if (center > 0) return center;
      }
    }
    // Fallback: first pan with a center frequency
    for (const [, pan] of this._panadapters) {
      const center = parseFloat(pan.center || 0);
      if (center > 0) return center;
    }
  ...
  /**
   * Send CW text via CWX.
   * The radio will key CW at the current transmit slice's WPM setting.
   * @param {string} text - CW text to send (e.g., "CQ CQ DE W1AW")
   */
  cwxSend(text) {
    if (!text) return;
    if (!this._cwBreakIn) {
      console.log('[SmartSDR] CWX blocked — CW break-in is OFF on radio');
      this.emit('cwx-blocked');
      return;
    }
    this._send(`cwx send "${text}"`);
  }

  /**
   * Clear the CWX buffer and stop sending.
   */
  cwxClear() {
  ...
  /**
   * Erase the last N characters from the CWX buffer.
   * @param {number} count - Number of characters to erase
   */
  cwxErase(count) {
    this._send(`cwx erase ${count || 1}`);
  }

  /**
   * Insert text at a specific position in the CWX buffer.
   * @param {number} index - Character position (1-based)
   * @param {string} text - Text to insert
   */
  cwxInsert(index, text) {
    if (!text) return;
    this._send(`cwx insert ${index} "${text}"`);
  }
